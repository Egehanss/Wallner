...
JIT_WARNING
	...
	function server(){
		...
		if(($pid = Filesystem::createLockFile($lockFilePath)) !== null){
			...
			critical_error(" " . VersionInfo::NAME . " adında (PID $pid) 'li sunucu şuanda bu klasörü kullanıyor (" . realpath($dataPath) . ").");

			critical_error("Yeni bir sunucu çalıştırmak için şuanda çalışan diğer sunucuyu kapatın lütfen.");
			exit(1);
		}
