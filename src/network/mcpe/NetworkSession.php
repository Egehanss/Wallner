...
class NetworkSession{
	...
	public function __construct(
		...
		$this->logger->info("" . TextFormat::GREEN . "ip adresi ile oturum açıldı");
	}

	private function getLogPrefix() : string{
		return "" . TextFormat::GREEN . "Ağ Oturumu: " . TextFormat::WHITE . $this->getDisplayName();
	}
	...
	private function tryDisconnect(\Closure $func, string $reason) : void{
		if($this->connected && !$this->disconnectGuard){
			...
			$this->logger->info("" . TextFormat::RED . "adlı oturum" . TextFormat::WHITE . " $reason " . TextFormat::RED . "nedeniyle kapatıldı");
	...
	public function tick() : void{
		if($this->info === null){
			if(time() >= $this->connectTime + 10){
				$this->disconnect("Oturum zaman aşımı");
			}
