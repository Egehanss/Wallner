...
class LoginPacketHandler extends PacketHandler{
	...
	public function handleLogin(LoginPacket $packet) : bool{
		...
		if(!$this->server->isWhitelisted($playerInfo->getUsername())){
			...
			$ev->setKickReason(PlayerPreLoginEvent::KICK_REASON_SERVER_WHITELISTED, "§cSunucu şuanda bakımda!");
		}
		if($this->server->getNameBans()->isBanned($playerInfo->getUsername()) || $this->server->getIPBans()->isBanned($this->session->getIp())){
			$ev->setKickReason(PlayerPreLoginEvent::KICK_REASON_BANNED, "§cSunucumuzdan Yasaklandınız!");
	...
	protected function isCompatibleProtocol(int $protocolVersion) : bool{
		...
		if($protocolVersion === ProtocolInfo::PROTOCOL_1_19_20){
			return $protocolVersion === ProtocolInfo::PROTOCOL_1_19_20;
		}elseif($protocolVersion === ProtocolInfo::CURRENT_PROTOCOL){
