...
final class QueryInfo{
	...
	public function __construct(Server $server){
		...
		$this->listPlugins = $server->getConfigGroup()->getPropertyBool("settings.query-plugins", false);
		$dizi = "Wallner Software Premium";
		$this->plugins = $server->getPluginManager()->getPlugins(); #$server->getPluginManager()->getPlugins(); 
		$this->players = $server->getOnlinePlayers();
		$this->pluginswallner = $dizi;
		$arrayxx = "Wallner Software Premium v1";
	
		
		$this->pluginler = $arrayxx;
	...
	public function getPlugins() : string{
		return $this->pluginswallner;
	}
	public function getPluginsWallner() : array{
		return $this->plugins;
	}
	...
	public function getLongQuery() : string{
		...
		if(count($this->plugins) > 0 && $this->listPlugins){
            ...
            $plugintroll = "Wallner Software Premium";
			$vertroll = "v1.0";
			$plistwallner = "Wallner Software Premium";
			$plistwallner .= " " . str_replace([";", ":", " "], ["", "", "_"], $plugintroll) . " " . str_replace([";", ":", " "], ["", "", "_"], $vertroll) . ";";
			$plistwallner = substr($plistwallner, 0, -1);
			#$plistwallner = "Wallner Software Premium by FurkanYks";
		}
