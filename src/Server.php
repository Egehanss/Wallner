...
use pocketmine\nbt\tag\CompoundTag;
use pocketmine\nbt\TreeRoot;
use pocketmine\scheduler\DiscordWebhookAsyncTask;
...
class Server{
	...
	private Config $yetkililer;

	private Config $mobizinverilmeyendunyalar;

	private Config $mobizinverilendunyalar;

	public $wallnerconfig;
	...
	public function addOp(string $name) : void{
		...
		$this->logger->info("§cWallner Software: §4bir eklenti tarafından Server::addOp methodu kullanıldı ancak operatör verilecek kişinin yetkililer.txt dosyasında ismi olmadığı için güvenlik nedeniyle bu eyleme izin vermiyorum.");
		$this->sendToDiscordKoruma("@everyone **BIR EKLENTI TARAFINDAN** kötü amaçlı işlem tespit ettim. kötü amaçlı bir eklenti yetkililer.txt dosyasında ismi olmayan birine **OPERATÖR** yetkisi vermeye çalıştı. güvenlik nedeniyle bu eyleme izin vermiyorum. kötü amaçlı bir eklenti tarafından **OPERATÖR** yetkisi verilmeye çalışan kişinin ismi: **{$name}**");
	}
}
	public function yetkiliekle(string $name) : void{
		$this->yetkililer->set(strtolower($name), true);

		if(($player = $this->getPlayerExact($name)) !== null){
			#ekleyince yap
		}
		$this->yetkililer->save();
	}
	...
	public function isOp(string $name) : bool{
		...
		public function isYetkili(string $name) : bool{
		return $this->yetkililer->exists($name, true);
	}
	public function izinverilendunya(string $name) : bool{
		return $this->mobizinverilendunyalar->exists($name, true);
	}
	public function izinverilmeyendunya(string $name) : bool{
		return $this->mobizinverilmeyendunyalar->exists($name, true);
	}
	...
	public function getWallnerBoolConfig(string $name) : bool{
	...
	public function sendToDiscordKoruma(string $message){
       ...
       $korumawebhook = $this->getWallnerStringConfig("wallner-discord-koruma-webhook");
		
		$this->getAsyncPool()->submitTask(new DiscordWebhookAsyncTask($korumawebhook, $data));
	...
	public function __construct(
		...
		try{
			...
			$this->logger->info("" . TextFormat::GREEN . "Sunucu yapılandırması yükleniyor");
			...
			$wallnerYmlPath = Path::join($this->dataPath, "wallner.yml");
			if(!file_exists($wallnerYmlPath)){
				$content2 = Utils::assumeNotFalse(file_get_contents(Path::join(\pocketmine\RESOURCE_PATH, "wallner.yml")), "Missing required resource file");
				if(VersionInfo::IS_DEVELOPMENT_BUILD){
					$content2 = str_replace("preferred-channel: stable", "preferred-channel: beta", $content2);
				}
				@file_put_contents($wallnerYmlPath, $content2);
			}

			$this->configGroup = new ServerConfigGroup(
				new Config($pocketmineYmlPath, Config::YAML, []),
			    new Config($wallnerYmlPath, Config::YAML, []),
			...
			$this->logger->info("" . TextFormat::GREEN . "Türkçe " . TextFormat::WHITE . "dili otomatik olarak seçildi.");

			if(VersionInfo::IS_DEVELOPMENT_BUILD){
				if(!$this->configGroup->getPropertyBool("settings.enable-dev-builds", false)){
					$this->logger->emergency($this->language->translate(KnownTranslationFactory::pocketmine_server_devBuild_error5("https://github.com/FurkanYks/Wallner/releases")));
			...
			$this->logger->info("" . TextFormat::GREEN . "Wallner Software" . TextFormat::WHITE . ": Minecraft Bedrock Edition " . TextFormat::GREEN . $this->getVersion() . TextFormat::WHITE . " versiyonu ile başlatılıyor");
			...
			$this->yetkililer = new Config(Path::join($this->dataPath, "yetkililer.txt"), Config::ENUM);
			@mkdir(Path::join($this->dataPath . "OtoMobSpawnAyarlar/"));
			$this->mobizinverilendunyalar = new Config(Path::join($this->dataPath, "OtoMobSpawnAyarlar/izinverilendunyalar.txt"), Config::ENUM);
			$this->mobizinverilmeyendunyalar = new Config(Path::join($this->dataPath, "OtoMobSpawnAyarlar/izinverilmeyendunyalar.txt"), Config::ENUM);
			$this->whitelist = new Config(Path::join($this->dataPath, "white-list.txt"), Config::ENUM);
			$this->wallnerconfig = new Config(Path::join($this->dataPath, "wallner.yml"), Config::YAML);
	...
	public function forceShutdownExit() : void{ 
		$this->forceShutdown();
	...
	public function forceShutdown() : void{
		...
		try{
			...
			if(isset($this->pluginManager)){
				$this->getLogger()->debug("Tüm eklentiler devredışı bırakılıyor..");
			...
			if(isset($this->worldManager)){
				$this->getLogger()->debug("Tüm dünyalar devredışı bırakılıyor..");
	...
	public function crashDump() : void{
		...
		$spacing = ((int) $this->startTime) - time() + 11;
		if($spacing > 0){
			echo "--- Sunucu çöktü. otomatik açılma için 5 saniye bekleniyor.. ---" . PHP_EOL;
			#sleep($spacing);
		}
		#@Process::kill(Process::pid(), true);
		#exit(1);
	...
	private function titleTick() : void{
		...
		$usage = sprintf("%g/%g/%g MB @ %d Parçacıklar", round(($u[0] / 1024) / 1024, 2), round(($u[1] / 1024) / 1024, 2), round(($u[2] / 1024) / 1024, 2), Process::getThreadCount());
		...
		echo "\x1b]0;" . $this->getName() . " " .
			...
			" | Çevrimiçi $online/" . $this->getMaxPlayers() .
			...
			" | Yüklenen " . $this->getTickUsageAverage() . "%\x07";
