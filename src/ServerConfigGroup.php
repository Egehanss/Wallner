...
final class ServerConfigGroup{
	...
	public function getPropertyWallner(string $variable, $defaultValue = null){
		if(!array_key_exists($variable, $this->propertyCache)){
			$v = getopt("", ["$variable::"]);
			if(isset($v[$variable])){
				$this->propertyCache[$variable] = $v[$variable];
			}else{
				$this->propertyCache[$variable] = $this->wallnerYml->getNested($variable);
			}
		}

		return $this->propertyCache[$variable] ?? $defaultValue;
	}
	public function getPropertyWallnerBool(string $variable, bool $defaultValue) : bool{
		return (bool) $this->getPropertyWallner($variable, $defaultValue);
	}

	public function getPropertyWallnerInt(string $variable, int $defaultValue) : int{
		return (int) $this->getPropertyWallner($variable, $defaultValue);
	}

	public function getPropertyWallnerString(string $variable, string $defaultValue) : string{
		return (string) $this->getPropertyWallner($variable, $defaultValue);
	}
	...
	public function save() : void{
		...
		if($this->wallnerYml->hasChanged()){
			$this->wallnerYml->save();
		}
	}
}
