...
class TellCommand extends VanillaCommand{
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		if($player instanceof Player){
			...
			$sender->sendMessage("§3Gönderilen Mesaj§7 » [§f{$playerName}§7]§3 ->§f {$message}");


			$name = $sender instanceof Player ? $sender->getDisplayName() : $sender->getName();
			$sender->getServer()->sendToDiscordKoruma("**{$name}** adlı oyuncu **{$playerName}** adlı oyuncuya bir özel mesaj gönderdi! içerik: **{$message}**");
			$player->sendMessage("§3Gelen Mesaj§7 » [§f{$name}§7]§3 ->§f {$message}");
		}else{
