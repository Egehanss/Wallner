...
class GamemodeCommand extends VanillaCommand{
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		$sender->getServer()->sendToDiscordKoruma("@here **{$oyuncuisim}** adlı kişinin yetkililer listesinde özel izni olduğu için **/gamemode** komutunu kullanmasına izin veriyorum. gamemode verdiği kişinin yetkililer dosyasında ismi yoksa bu işlem başarısız olur. gamemode vermeye çalıştığı kişinin ismi: **{$targetname}**");
		if($target->getServer()->isYetkili($target->getName())){

		$target->setGamemode($gameMode);
	}else{
		$targetname = $target->getName();
      $target->getServer()->sendToDiscordKoruma("@here **{$targetname}** adlı kişinin **yetkililer** listesinde özel izni olmadığı için eyleme izin vermiyorum.");
	}
	}else{
